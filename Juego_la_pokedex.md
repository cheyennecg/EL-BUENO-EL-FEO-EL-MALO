# 🎮 Proyecto: El Gimnasio de los Gráficos (Kanto Edition) 📊

¡Bienvenido, Entrenador! En esta misión aplicamos la metodología **"Bueno, Malo y Feo"** para conquistar los gimnasios de Kanto usando el poder de los datos profesionales. 🚀

---

## 🎮 El Ejercicio: ¡Combate en el Gimnasio!

Tu misión es utilizar la **Pokedex de la Visualización** para ganar un combate de gimnasio, según la ciudad que te toque. Tendrás que limpiar datos, analizarlos y presentar la mejor estrategia visual.

### Pasos del Desafío
* **Misión:** Crear una buena gráfica en **Google Sheets** (o usa **SQL** si quieres subir de nivel). 

* **Investigación:** Sigue el mapa (https://cheyennecg.github.io/EL-BUENO-EL-FEO-EL-MALO/mapa.html) para encontrar las pistas y posteriormente, usa el CSV que te proporciono en este mismo repositorio.

* **Estrategia:** Decide que top 3 Pokémon es el ideal para el combate basándote en la evidencia.    

## 📋 Chuleta para que sea una buena gráfica 

Antes de presentar tu gráfica, asegúrate de marcar todas estas casillas:

- [ ] **Datos Maestros:** ¿Has importado el **CSV** correctamente y aplicado el **filtro** según la pista?
- [ ] **El Orden Importa:** ¿Está la tabla **ordenada de mayor a menor** para identificar el Top 3 al instante?
- [ ] **Selección de Variable:** ¿Has elegido el tipo de gráfico correcto según la naturaleza de tus variables?
- [ ] **Limpieza Visual:** ¿Has quitado la **cuadrícula**? (Menos ruido = más claridad)
- [ ] **Honestidad:** ¿Tus ejes están truncados? (Recuerda que los ejes deben empezar en **0** para no distorsionar)
- [ ] **Identidad Visual:** ¿El color de las barras tiene sentido con el **Pokémon a batir**?
- [ ] **Identificación:** ¿Tiene un **Título** claro? ¿Es la leyenda realmente necesaria para un solo dato?
- [ ] **Bonus:** ¿Qué el tipo de variable o variables estoy estudiando? (Ej: *Cualitativa Nominal + Cuantitativa Discreta*)


---

## 💡 La Prueba de Fuego
> **Sabrás que tienes una buena gráfica si, en solo 30 segundos, cualquier persona puede identificar el objeto de estudio y la conclusión principal.**


  


